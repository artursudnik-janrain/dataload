...
class JsonBatchReader(BaseBatchReader):
    def __init__(self, json_file, batch_size=100, start_at=1, delimiter=","):
        super(JsonBatchReader, self).__init__()
    ...
    def utf8_validate(self):
        logger.info("Validating UTF-8 encoding")
        f = codecs.open(self.json_file, encoding='utf8', errors='strict')
        line_number = 1
        try:
            for i, line in enumerate(f):
                line_number += 1
    ...
    def __iter__(self):
        ...
        with open(self.json_file) as f:
            reader = json.load(f, encoding="utf-8")
            batch = []
            batch_number = 0
